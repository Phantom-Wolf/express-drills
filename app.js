...
// express is a framework for building web-servers in node
...
const app = express();
app.use(morgan('dev'));
// the .use() method is used precisely for mounting middleware
...
// (req, res) => {res.send('Hello Express!');}  <--- this is a request handler, see below
// req: an object represents HTTP request
// res: an object that represents an HTTP response after the request is processed
...
app.get('/pizza/pepperoni', (req, res) => {
	res.send('Here comes you pepperoni pizza...no bacon bits?');
});

app.get('/pizza/pineapple', (req, res) => {
	res.send('Leave...and never come back');
});
...
app.get('/sum', (req, res) => {
	...
	const total = `Adding ${a} and ${b} together equals ${c}.`;
...
app.get('/lotto', (req, res) => {
	...
	if (!numbers) {
		return res.status(400).send('Must provide a number');
	}
	...
	if (guesses.length != 6) {
		return res.status(400).send('Must choose 6 numbers, between 1 and 20');
	}
